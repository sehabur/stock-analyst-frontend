export function calculateSma(prices: any[], period: number) {
  let sma = [];
...
export function calculateEma(prices: any[], period: number) {
  const k = 2 / (period + 1);
  let emaArray = [];
  let sma = 0;
  ...
  for (let i = period; i < prices.length; i++) {
    const ema: any = prices[i] * k + emaArray[emaArray.length - 1] * (1 - k);
    emaArray.push(ema);
  }
...
export function calculateRsi(prices: any, period = 14) {
...
function calculateTrueRange(highs: any, lows: any, closes: any) {
  let trueRanges = [];
...
function calculateDirectionalMovement(highs: any, lows: any) {
  let plusDM = [];
  let minusDM = [];
  for (let i = 1; i < highs.length; i++) {
...
function calculateSmoothedValues(values: any, period: number) {
  let smoothedValues = [];
  let sum = values
    .slice(0, period)
...
export function calculateAdx(highs: any, lows: any, closes: any, period = 14) {
...
export function calculateWilliamsPercentR(
  highs: [],
  lows: [],
  closes: [],
  period = 14
...
function calculateTypicalPrice(highs: any, lows: any, closes: any) {
  return highs.map((high: any, i: any) => (high + lows[i] + closes[i]) / 3);
}

function calculateRawMoneyFlow(typicalPrices: any, volumes: any) {
  return typicalPrices.map((price: any, i: any) => price * volumes[i]);
}
...
export function calculateSmaLastValue(prices: any[], period: number) {
  const slicedPrices = prices.slice(-period);

  const sum = slicedPrices.reduce((total: number, current: any) => {
...
export function calculateEmaLastValue(prices: any[], period: number) {
  const k = 2 / (period + 1);
  let emaArray = [];
  let sma = 0;
...
export function calculateRsiLastValue(prices: any, period = 14) {
  if (prices.length < period) {
    return null;
  }
...
export function calculateStochasticKLastValue(
  prices: any,
  period = 14,
  smoothK = 3,
  smoothD = 3
...
export function calculateAdxLastValue(
  highs: any,
  lows: any,
  closes: any,
  period = 14
...
export function calculateMacdLastValue(
  prices: [],
  shortPeriod = 12,
  longPeriod = 26,
  signalPeriod = 9
...
export function calculateWilliamsPercentRLastValue(
  highs: [],
  lows: [],
  closes: [],
  period = 14
...
export function calculateMoneyFlowIndexLastValue(
  highs: any,
  lows: any,
  closes: any,
  volumes: any,
